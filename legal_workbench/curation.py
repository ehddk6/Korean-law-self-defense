...
def _additional_direct_conclusion(text: str) -> bool:
    return bool(
        re.search(
            ...
            r"(?:혈압(?:상승|의\s*상승)|혈압상승).{0,120}(?:상병|뇌출혈).{0,120}(?:이어졌다고|직접적인\s*원인이|초래한|발생한)|"
            r"처분의\s*위법사유를\s*구성할\s*수\s*없|"
            r"(?:원고의\s*)?(?:이\s*부분\s*)?주장은\s*타당하지\s*않|"
            r"자연적(?:인)?\s*경과.{0,120}(?:급격한\s*변동|급격히\s*악화).{0,100}(?:평가할\s*수|인정)|"
            r"원심의\s*판단.{0,40}(?:수긍|어렵)|원심.{0,100}(?:항고심|자료).{0,100}(?:판단했어야|심리.{0,30}판단했어야)|"
            r"업무상.{0,120}(?:돌발상황|부담).{0,120}(?:뇌출혈|뇌경색|상병).{0,100}(?:상당인과|인정)|"
            r"(?:변제기간.{0,300})?(?:착오로\s*말미암아\s*잘못\s*기재|잘못\s*기재된\s*것으로).{0,100}(?:충분|할\s*것이다)|"
            r"(?:항고심|원심).{0,160}(?:서면|자료).{0,120}(?:토대로|기초로).{0,100}(?:판단했어야|심리했어야)|"
            r"(?:서면과\s*)?자료를\s*토대로.{0,120}판단하였어야\s*할\s*것이다|"
            r"보증금.{0,180}(?:반환되었는지|반환받은).{0,180}(?:심리하여|재산인지)|"
            r"(?:파산|회생).{0,120}(?:절차.{0,40}(?:종료|종결)|면책신청.{0,80}(?:종기|기간))|"
            r"(?:항고심|원심).{0,100}(?:보완자료|추가자료).{0,120}(?:판단했어야|심리했어야)|"
            r"오기.{0,100}(?:충분|명백).{0,100}(?:사안|인정)|보증금.{0,120}(?:반환|공제|사용).{0,120}(?:재산성|심리|판단)했어야|"
            r"(?:판시\s*)?범죄사실|유죄\s*전제|양형\s*판단|신상정보\s*등록|"
            r"(?:추행에\s*해당하며|추행에\s*해당).{0,100}고의도\s*인정|"
            r"(?:신상정보|공개·고지명령).{0,140}(?:아니|면제|특별한\s*사정).{0,80}(?:판단|인정)|"
            r"(?:피고인|피고).{0,100}(?:주장|변호인).{0,100}받아들이지\s*않|"
            r"(?:법률상\s*)?처단형.{0,100}(?:범위|벌금)|이\s*사건\s*범행.{0,100}(?:양형|판단|책임)|"
            r"개인정보.{0,120}(?:누설|유출).{0,100}(?:볼\s*수도\s*없|해당하지)|"
            r"처벌대상이\s*된다고\s*볼\s*수는\s*없|"
            r"사회상규에\s*위배되는\s*행위라고\s*보기도\s*어렵|"
            r"성적\s*자유를\s*침해하는\s*행위로서\s*추행에\s*해당|"
            r"절차적\s*정의에\s*반한다고\s*볼만한\s*사정을\s*찾아보기\s*어렵|"
            r"이로써\s*피고인은.{0,180}(?:추행|개인정보).{0,180}(?:하였다|이용하였다|제공하였다)|"
            r"원심은.{0,180}(?:변론재개|석명|보호조치).{0,180}(?:주장을\s*배척|위반하였다고\s*보|인정하기에\s*부족)|"
            r"(?:변론재개|보호조치|DLP|FTP).{0,220}(?:위반하였다고\s*인정하기에\s*부족|위반하였다고\s*볼\s*수|위반하였다고\s*단정하기|의무를\s*부담하지\s*아니)|"
            r"(?:석명|지적)의무.{0,160}(?:위반|심리).{0,100}(?:위법|제대로\s*하지|배척)",
            text,
            flags=re.M,
        )
    )
    ...
    if "대법원" in court:
        ...
        if facts_heading:
            ...
            target_heading = re.search(
                r"(?im)^\s*(?:(?:\d+|[가-하])\.?\s*)?(?:(?:이\s*법원의\s*판단|당심의\s*판단|법원의\s*판단|판단)|"
                r"(?:【\s*)?피고인과\s*변호인의\s*주장에\s*대한\s*판단(?:\s*】)?(?:\s+\d+\..*)?|"
                r"(?:【\s*)?법령의\s*적용(?:\s*】)?(?:\s+\d+\..*)?)\s*$",
                text,
            )
...
def _supreme_blind_context(text: str) -> str:
    ...
    if facts_intro:
        ...
        next_analysis = re.search(r"(?m)^\s*[3-9]\.\s*(?:판단|결론)\s*$", following)
        if next_analysis:
            return text[facts_intro.start() : facts_intro.end() + next_analysis.start()]

    next_main = re.search(r"(?m)^\s*3\.\s*(?:판단|결론)\s*$", text)
    if next_main:
        return text[: next_main.start()]
